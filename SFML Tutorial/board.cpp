...
bool Board::kingMove(Move move) {
	if (move.dx == 2) {
		if (whiteToMove && !whiteKing->moved) {
			...
			if (isKingAttacked(6, whiteKing->pos, whiteKing->pos + 2)) return false;
			...
			if (isKingAttacked(12, blackKing->pos, blackKing->pos + 2)) return false;
...
Piece* Board::makeMove(Move move, Piece* piece) {
	Piece* captured = getPieceAt(move.end);
	if (captured) captured->pos = -1;

	board[move.start] = 0;
	board[move.end] = piece->id;
	piece->pos = move.end;
	whiteToMove = !whiteToMove;
	return captured;
}
void Board::undoMove(Move move, Piece* piece, Piece* captured) {
	if (captured) {
		captured->pos = move.end;
		board[move.end] = captured->id;
	}
	else board[move.end] = 0;
	board[move.start] = piece->id;
...
int Board::search(int depth, int alpha, int beta) {
	...
	Piece* king;
	if (whiteToMove) king = whiteKing;
	else king = blackKing;
	std::vector<Move> moves = getLegalMoves();
	if (!moves.size()) {
		if (isKingAttacked(king->id, king->pos, king->pos)) return -2147483647;
		else return 0;
	}
	...
	for (int i = 0; i < moves.size(); i++) {
		mover = getPieceAt(moves[i].start);
		captured = makeMove(moves[i], mover);
		int eval = -1 * search(depth - 1, -beta, -alpha);
		undoMove(moves[i], mover, captured);
		if (eval >= beta) {
			return beta;
		}
...
void Board::orderMoves(std::vector<Move> moves) {
	int size = moves.size();
	int a = 0;
...
Move Board::findMove(int depth) {
	std::vector<Move> moves = getLegalMoves();
	if (!moves.size()) return Move(-1, -1);
	...
	for (int i = 0; i < moves.size(); i++) {
		Piece* mover = getPieceAt(moves[i].start);
		Piece* captured = makeMove(moves[i], mover);
		int eval = -1 * search(depth - 1, -2147483647, 2147483647);
		if (eval > best) {
			best = eval;
			move = moves[i];
		}
