...
Piece::Piece(int id, int pos, Board* board) : id(id), pos(pos) {
...
bool Board::isLegalMove(Piece* mover, int pos) {
	...
	int a = moves.size();
	for (int i = 0; i < a; i++) {
		std::cout << moves[i].start << " > " << moves[i].end << std::endl;
	}
	if (!a) {
		if (theSoundToPlay == 2) end = 1;
		else end = 2;
	}
...
bool Board::whitePawnMove(Move move) {
	if (!move.dx) {
		...
		if (move.dy == 1) return true;
		return false;
	}
	else if ((move.dx == 1 || move.dx == -1) && move.dy == 1) {
		if (board[move.end] || move.end == enPassantable) return true;
		return false;
	}
	return false;
}
